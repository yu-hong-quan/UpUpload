...
router.post('/merge', async ctx => {
  ...
  // 最终生成的文件路径+名称
  const filePath = path.resolve('', 'public', `${hash}.${ext}`)
...
router.post('/check', async ctx => {
  ...
  const filePath = path.resolve('UpUpload_Server', 'public', `${hash}.${ext}`)
  let uploaded = false
  let uploadedList = []
  if (fse.existsSync(filePath)) {
    // 文件已存在
    ...
    uploadedList = await getUploadedList(path.resolve('UpUpload_Server', 'public', hash))
  }
  ctx.body = {
    uploaded,
    uploadedList
...
router.post('/delete', async ctx => {
  ...
  const filePath = path.resolve('UpUpload_Server', 'public', name)
